...
export async function getDashboardData() {
  ...
  const [
    ...
    supabase
      .from("farmers")
      .select("gender, beneficiary_type, district_id")
      .limit(5000),
    ...
    supabase
      .from("campaigns")
      .select(
        "id, name, allocated_farmers, delivered_count, total_farmers, status",
      )
      .in("status", ["Active", "Approved"])
      .order("created_at", { ascending: false })
      .limit(8),
  ...
  const farmerStatusChart = Object.entries(statusMap).map(([name, value]) => ({
    name,
    value,
  }));
  ...
  const campaignCompletionChart = Object.entries(campStatusMap).map(
    ([name, value]) => ({ name, value }),
  );

  const whStockMap: Record<number, number> = {};
  for (const s of stockLedger.data ?? []) {
    whStockMap[s.warehouse_id] =
      (whStockMap[s.warehouse_id] ?? 0) + Number(s.quantity);
  }
  ...
  if (whIds.length) {
    const { data: whs } = await supabase
      ...
      .select("id,name")
      .in("id", whIds);
    warehouseStockChart = (whs ?? []).map((w: any) => ({
      name: w.name,
      stock: Math.max(0, whStockMap[w.id] ?? 0),
    }));
  }

  const podByDay: Record<
    string,
    { date: string; verified: number; pending: number }
  > = {};
  for (let i = 6; i >= 0; i--) {
  ...
  const femaleCount = farmerRows.filter(
    (r: any) => r.gender === "Female",
  ).length;
  const femalePct =
    farmerRows.length > 0
      ? Math.round((femaleCount / farmerRows.length) * 100)
      : 0;
  const indivCount = farmerRows.filter(
    (r: any) => r.beneficiary_type === "individual",
  ).length;
  const groupCount = farmerRows.filter(
    (r: any) => r.beneficiary_type === "group",
  ).length;
  ...
  const campaignProgress = (activeCampaignRows.data ?? []).map((c: any) => {
    const target = Math.max(
      Number(c.allocated_farmers) || 0,
      Number(c.total_farmers) || 0,
      1,
    );
  ...
  if (lowStockRows.length) {
    const [whMap2, itemMap2] = await Promise.all([
      lookupMap(
        ...
        [
          ...new Set(
            lowStockRows.map((r: any) => r.warehouse_id).filter(Boolean),
          ),
        ],
        "id,name",
      ),
      lookupMap(
        ...
        [
          ...new Set(
            lowStockRows.map((r: any) => r.input_item_id).filter(Boolean),
          ),
        ],
        "id,name,unit",
      ),
...
export async function checkFarmerDuplicate(
  ...
  duplicate: boolean;
  existingFarmer?: { id: number; name: string };
}> {
  ...
  const matches = (json.duplicates ?? []) as Array<{
    id: number;
    phone: string;
    nationalId: string;
    farmerCode: string;
  ...
  return {
    ...
    existingFarmer: {
      id: match.id,
      name: match.name || match.farmerCode || "Unknown",
    },
  };
}
...
export async function listFarmers(
  ...
  const [districtMap, chiefdomMap, vcMap] = await Promise.all([
    lookupMap(
      "districts",
      [...new Set(rows.map((r: any) => r.district_id).filter(Boolean))],
      "id,name",
    ),
    lookupMap(
      "chiefdoms",
      [...new Set(rows.map((r: any) => r.chiefdom_id).filter(Boolean))],
      "id,name",
    ),
    lookupMap(
      "value_chains",
      [...new Set(rows.map((r: any) => r.value_chain_id).filter(Boolean))],
      "id,name",
    ),
  ]);
  return {
...
export async function getFarmerTypeCounts(): Promise<{
  individuals: number;
  groups: number;
}> {
  const [ind, grp] = await Promise.all([
    supabase
      .from("farmers")
      .select("*", { count: "exact", head: true })
      .eq("beneficiary_type", "individual"),
    supabase
      .from("farmers")
      .select("*", { count: "exact", head: true })
...
export async function getFarmer(id: number) {
  ...
  const [districtMap, chiefdomMap, sectionMap, vcMap] = await Promise.all([
    ...
    lookupMap(
      "value_chains",
      r.value_chain_id ? [r.value_chain_id] : [],
      "id,name",
    ),
  ]);
  return {
...
export async function createFarmer(payload: any) {
  ...
  await logAudit(
    "CREATE",
    "farmers",
    `Registered farmer ${(data as any).farmer_code}`,
    "farmer",
    (data as any).id,
  );
  return cc(data);
...
export async function updateFarmer(id: number, payload: any) {
  const { data, error } = await supabase
    ...
    .update({
      ...
      last_name: payload.lastName,
      gender:
        payload.beneficiaryType === "group" ? "N/A" : (payload.gender ?? null),
      phone: payload.phone ?? null,
      national_id: payload.nationalId ?? null,
      district_id: payload.districtId ?? null,
      chiefdom_id: payload.chiefdomId ?? null,
...
// ── CAMPAIGNS ──────────────────────────────────────────────────────────────
async function campaignApi(path: string, init: RequestInit = {}) {
  const {
    data: { session },
  } = await supabase.auth.getSession();
  ...
  if (!response.ok) {
    const details = Array.isArray((result as any).details)
      ...
      : "";
    throw new Error(
      `${(result as any).error ?? "Campaign request failed."}${details}`.trim(),
    );
  }
  return result;
}

export async function listCampaigns(page = 1, limit = 20, status?: string) {
  const params = new URLSearchParams({
    page: String(page),
    limit: String(limit),
  });
  if (status) params.set("status", status);
  return campaignApi(`/api/campaigns?${params}`);
}

export async function getCampaign(id: number) {
  return campaignApi(`/api/campaigns/${id}`);
}

export async function createCampaign(payload: any) {
  return campaignApi("/api/campaigns", {
    method: "POST",
...
export async function deleteCampaign(id: number) {
  return campaignApi(`/api/campaigns/${id}`, { method: "DELETE" });
}

function transitionCampaign(id: number, action: string, reason?: string) {
  return campaignApi(`/api/campaigns/${id}/${action}`, {
    method: "POST",
    body: JSON.stringify(reason ? { reason } : {}),
  });
}

export const submitCampaign = (id: number) => transitionCampaign(id, "submit");
export const approveCampaign = (id: number) =>
  transitionCampaign(id, "approve");
export const rejectCampaign = (id: number, reason: string) =>
  transitionCampaign(id, "reject", reason);
export const cancelCampaign = (id: number, reason: string) =>
  transitionCampaign(id, "cancel", reason);
export const completeCampaign = (id: number) =>
  transitionCampaign(id, "complete");
...
export async function listAllocations(
  ...
  return campaignApi(`/api/allocations?${params}`);
}

export async function createAllocation(payload: any) {
  return campaignApi("/api/allocations", {
    method: "POST",
    body: JSON.stringify(payload),
  });
}
...
export async function removeAllocation(id: number) {
  return campaignApi(`/api/allocations/${id}`, { method: "DELETE" });
}
...
export async function updateAllocation(
  ...
  return campaignApi(`/api/allocations/${id}`, {
    method: "PATCH",
    body: JSON.stringify(payload),
  });
}
...
export async function getStockBalance() {
  ...
  const [whMap, itemMap] = await Promise.all([
    ...
    lookupMap(
      "input_items",
      [...new Set(rows.map((r: any) => r.input_item_id).filter(Boolean))],
...
export async function receiveStock(payload: any) {
  ...
  const result = await response
    ...
    .catch(() => ({ error: response.statusText }));
  if (!response.ok)
    throw new Error((result as any).error ?? "Failed to receive stock");
  return result;
}
...
export async function createProcurementOrder(payload: any) {
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      order_date: payload.orderDate
        ? new Date(payload.orderDate).toISOString()
        : null,
      expected_delivery: payload.expectedDelivery
        ? new Date(payload.expectedDelivery).toISOString()
        : null,
  ...
  await logAudit(
    "CREATE",
    "procurement",
    `Created PO ${(data as any).order_code}`,
    "procurement",
    (data as any).id,
  );
  return cc(data);
...
export async function createVehicle(payload: any) {
  ...
  await logAudit(
    "CREATE",
    "vehicles",
    `Created vehicle ${(data as any).vehicle_code}`,
    "vehicle",
    (data as any).id,
  );
  return cc(data);
...
export async function createDriver(payload: any) {
  ...
  await logAudit(
    "CREATE",
    "drivers",
    `Created driver ${(data as any).driver_code}`,
    "driver",
    (data as any).id,
  );
  return cc(data);
...
export async function listDispatches(
  ...
  if (manifestCode && manifestCode.trim())
    params.set("manifestCode", manifestCode.trim());
  if (showArchived) params.set("archived", "true");
  const resp = await fetch(apiUrl(`/api/dispatch?${params}`), {
    headers: { Authorization: `Bearer ${token}` },
  });
  if (!resp.ok)
    throw new Error(`Failed to list dispatches: ${resp.statusText}`);
  return resp.json();
}
...
export async function removeDispatchItem(dispatchId: number, itemId: number) {
  ...
  await logAudit(
    "DELETE",
    "dispatch",
    `Removed item #${itemId} from manifest #${dispatchId}`,
    "dispatch",
    dispatchId,
  );
}
...
export async function addDispatchItem(payload: any) {
  ...
  const resp = await fetch(
    apiUrl(`/api/dispatch/${payload.dispatchId}/items`),
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
...
export async function listGpsRoutes(params?: {
  ...
  const resp = await fetch(apiUrl(`/api/gis/routes?${qs}`), {
    headers: { Authorization: `Bearer ${token}` },
  });
  if (!resp.ok) {
    const err = await resp.json().catch(() => ({ error: resp.statusText }));
...
export function buildGisExportUrl(
...
export async function listVehicleGpsStatus() {
  ...
  const resp = await fetch(apiUrl("/api/gps/vehicles"), {
    headers: { Authorization: `Bearer ${token}` },
  });
  if (!resp.ok)
    throw new Error(`GPS vehicles fetch failed: ${resp.statusText}`);
  return resp.json();
}
...
export async function listGpsTrack(vehicleId?: number, limit = 50) {
  ...
  const token = await gpsToken();
  const resp = await fetch(
    apiUrl(`/api/gps/track/${vehicleId}?limit=${limit}`),
    { headers: { Authorization: `Bearer ${token}` } },
  );
...
export async function listGpsVehicleHistory(vehicleId: number, limit = 20) {
  const token = await gpsToken();
  const resp = await fetch(
    apiUrl(`/api/gps/history/${vehicleId}?limit=${limit}`),
    { headers: { Authorization: `Bearer ${token}` } },
  );
...
export async function listGpsTraceDevices(): Promise<{
  configured: boolean;
  devices: any[];
  vehicles: any[];
}> {
  const token = await gpsToken();
  const resp = await fetch(apiUrl("/api/gpstrace/devices"), {
    headers: { Authorization: `Bearer ${token}` },
  });
  if (!resp.ok)
    throw new Error(`GPS-Trace devices fetch failed: ${resp.statusText}`);
  return resp.json();
}

export async function syncGpsTrace(): Promise<{
  synced: number;
  total: number;
  linked: number;
}> {
  const token = await gpsToken();
  const resp = await fetch(apiUrl("/api/gpstrace/sync"), {
    method: "POST",
    headers: { Authorization: `Bearer ${token}` },
  });
...
export async function linkGpsTraceDevice(
  vehicleId: number,
  deviceId: string,
  deviceName?: string,
) {
...
export async function listPod(
  ...
  const [farmerMap, campaignMap, inputItemMap] = await Promise.all([
    lookupMap(
      "farmers",
      [...new Set(rows.map((r: any) => r.farmer_id).filter(Boolean))],
      "id,first_name,last_name,farmer_code,beneficiary_type,farmer_group,group_size,photo_url",
    ),
    lookupMap(
      "campaigns",
      [...new Set(rows.map((r: any) => r.campaign_id).filter(Boolean))],
      "id,name,campaign_code",
    ),
    lookupMap(
      "input_items",
      [...new Set(rows.map((r: any) => r.input_item_id).filter(Boolean))],
  ...
  return {
    data: rows.map((r: any) => ({
      ...
      vehicleGpsStatus:
        r.vehicle_gps_snapshot?.status ?? r.vehicle_gps_status ?? "Pending",
...
export async function getPodItems(podId: number) {
  const { data, error } = await supabase
    .from("pod_items")
    .select(
      "id, input_item_id, quantity_delivered, input_items(name, unit, category)",
    )
    .eq("pod_id", podId)
    .order("id");
  if (error) return [];
...
export async function createPod(payload: any) {
  ...
  const items = Array.isArray(payload.items)
    ? payload.items.map((item: any) => ({
        ...
        quantity: Number(
          item.quantity ?? item.quantityDelivered ?? item.quantity_delivered,
        ),
      }))
    : payload.inputItemId != null && payload.quantityDelivered != null
      ? [
          {
            inputItemId: Number(payload.inputItemId),
            quantity: Number(payload.quantityDelivered),
          },
        ]
      : undefined;
  const resp = await fetch(apiUrl("/api/pod"), {
    ...
    body: JSON.stringify({
      ...
      quantityDelivered:
        payload.quantityDelivered != null
          ? Number(payload.quantityDelivered)
          : undefined,
      items,
...
export async function listReconciliations() {
  ...
  const [dispatchMap, whMap] = await Promise.all([
    lookupMap(
      "dispatches",
      [...new Set(rows.map((r: any) => r.dispatch_id).filter(Boolean))],
      "id,manifest_code",
    ),
    lookupMap(
      "warehouses",
...
export async function createReconciliation(payload: any) {
  const userId = await intUid();
  const variance =
    (payload.loadedQuantity ?? 0) -
    (payload.deliveredQuantity ?? 0) -
    (payload.returnedQuantity ?? 0) -
...
export async function getFarmerBeneficiaryReport() {
  const { data, error } = await supabase
    .from("farmers")
    .select(
      "status, district_id, value_chain_id, gender, beneficiary_type, age_group, farm_size",
    )
    .limit(5000);
  if (error) throw new Error(error.message);
  const rows = data ?? [];
  ...
  const reportRows = Object.values(byDistrict)
  ...
  const individual = rows.filter(
    (r: any) => !r.beneficiary_type || r.beneficiary_type === "individual",
  ).length;
  ...
  const totalFarmSize =
    Math.round(reportRows.reduce((s, r) => s + (r.farmSize ?? 0), 0) * 10) / 10;
  return {
    rows: reportRows,
    vcRows,
    ageRows,
...
export async function getMEReport() {
  const [campaignsRes, podsRes] = await Promise.all([
    supabase
      .from("campaigns")
      .select(
        "id,name,campaign_code,season,total_farmers,allocated_farmers,delivered_count,status",
      )
      .order("created_at", { ascending: false })
      .limit(200),
    supabase
      .from("pod")
      .select(
        "farmer_id,campaign_id,otp_status,face_status,gps_status,status,quantity_delivered",
      )
      .limit(5000),
  ]);
  ...
  const farmerIds = [
    ...new Set(pods.map((p: any) => p.farmer_id).filter(Boolean)),
  ] as number[];
  ...
  const campaignRows = campaigns.map((c: any) => {
    ...
    const femalePods = cPods.filter(
      (p: any) => farmerMap[p.farmer_id]?.gender === "Female",
    );
    ...
    return {
      ...
      femalePct:
        cPods.length > 0
          ? Math.round((femalePods.length / cPods.length) * 100)
          : 0,
    };
  });
  ...
  const otpVerified = pods.filter(
    (p: any) => p.otp_status === "Verified",
  ).length;
  const faceVerified = pods.filter(
    (p: any) => p.face_status === "Verified",
  ).length;
  const gpsCaptured = pods.filter(
    (p: any) =>
      p.gps_status && p.gps_status !== "None" && p.gps_status !== "Pending",
  ).length;
  const podVerified = pods.filter(
    (p: any) => p.status === "Verified" || p.status === "Approved",
  ).length;
  const femaleDeliveries = pods.filter(
    (p: any) => farmerMap[p.farmer_id]?.gender === "Female",
  ).length;
  ...
  const vcDeliveryRows = Object.values(byVc)
    ...
    .map((r: any) => ({
      ...
      femalePct:
        r.deliveries > 0 ? Math.round((r.female / r.deliveries) * 100) : 0,
      verifiedPct:
        r.deliveries > 0 ? Math.round((r.verified / r.deliveries) * 100) : 0,
    }));
  ...
  return {
    ...
    verification: {
      podCount,
      otpVerified,
      otpRate: podCount > 0 ? Math.round((otpVerified / podCount) * 100) : 0,
      faceVerified,
      faceRate: podCount > 0 ? Math.round((faceVerified / podCount) * 100) : 0,
      gpsCaptured,
      gpsRate: podCount > 0 ? Math.round((gpsCaptured / podCount) * 100) : 0,
      podVerified,
      verifiedRate:
        podCount > 0 ? Math.round((podVerified / podCount) * 100) : 0,
      femaleDeliveries,
      femaleRate:
        podCount > 0 ? Math.round((femaleDeliveries / podCount) * 100) : 0,
    },
...
export async function getDistributionReport(from?: string, to?: string) {
  ...
  return rows.map((r: any) => ({
    ...
    completionPct:
      r.total_packages > 0
        ? Math.round((r.delivered_packages / r.total_packages) * 100)
        : 0,
  }));
}
...
export async function getConsolidatedDistributionReport(opts: {
  ...
  const campaignIds = [
    ...new Set(dRows.map((d: any) => d.campaign_id).filter(Boolean)),
  ] as number[];
  const warehouseIds = [
    ...new Set(dRows.map((d: any) => d.warehouse_id).filter(Boolean)),
  ] as number[];
  const vehicleIds = [
    ...new Set(dRows.map((d: any) => d.vehicle_id).filter(Boolean)),
  ] as number[];
  const driverIds = [
    ...new Set(dRows.map((d: any) => d.driver_id).filter(Boolean)),
  ] as number[];
  ...
  const inputItemIds = [
    ...new Set(allItems.map((i: any) => i.input_item_id).filter(Boolean)),
  ] as number[];
  const farmerIds = [
    ...new Set(allPods.map((p: any) => p.farmer_id).filter(Boolean)),
  ] as number[];
  ...
  const [inputItemMap, farmerMap] = await Promise.all([
    ...
    farmerIds.length
      ? lookupMap(
          "farmers",
          farmerIds,
          "id,farmer_code,first_name,last_name,gender,phone,beneficiary_type,farmer_group",
        )
      : Promise.resolve<Record<number, any>>({}),
  ]);
  ...
  return dRows.map((d: any) => {
    ...
    return {
      ...
      completionPct:
        d.total_packages > 0
          ? Math.round((d.delivered_packages / d.total_packages) * 100)
          : 0,
      items,
      pods,
      stats: {
        totalLoaded: items.reduce(
          (s: number, i: any) => s + i.quantityLoaded,
          0,
        ),
        totalDelivered: items.reduce(
          (s: number, i: any) => s + i.quantityDelivered,
          0,
        ),
        podCount: pods.length,
        verifiedCount: pods.filter((p: any) => p.status === "Verified").length,
        pendingCount: pods.filter((p: any) => p.status === "Pending").length,
        exceptionCount: pods.filter((p: any) => p.status === "Exception")
          .length,
      },
    };
  });
...
export async function listAllCampaigns() {
  const { data, error } = await supabase
    .from("campaigns")
    .select("id,name,campaign_code,season")
    .order("created_at", { ascending: false })
    .limit(200);
  if (error) throw new Error(error.message);
  return (data ?? []).map((r) => ({
    ...
    campaignCode: r.campaign_code as string,
...
export async function listAuditLogs(
  ...
  let q = supabase
    ...
    .range((page - 1) * limit, page * limit - 1);
  if (filters.search)
    q = q.ilike("description", `%${filters.search}%`) as typeof q;
  ...
  if (filters.toDate)
    q = q.lte("created_at", filters.toDate + "T23:59:59") as typeof q;
  const { data, error, count } = await q;
  if (error) throw new Error(error.message);
  return { data: cc(data ?? []), total: count ?? 0 };
}
...
export async function createUser(payload: any) {
  ...
  if (!resp.ok) {
    const err = await resp.json().catch(() => ({}));
    throw new Error(
      (err as any).error ?? (err as any).message ?? "Failed to create user",
    );
  }
  return resp.json();
}
...
export async function resetUserPassword(
  ...
  const token = await userApiToken();
  const resp = await fetch(
    apiUrl(`/api/users/profile/${profileId}/reset-password`),
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
...
export async function createDistrict(payload: { name: string; code: string }) {
  return mdFetch("/api/master-data/districts", {
    method: "POST",
    body: JSON.stringify(payload),
  });
}
export async function updateDistrict(
  ...
  return mdFetch(`/api/master-data/districts/${id}`, {
    method: "PUT",
    body: JSON.stringify(payload),
  });
}
...
export async function listChiefdoms(districtId?: number) {
  const url = districtId
    ? `/api/master-data/chiefdoms?districtId=${districtId}`
    : "/api/master-data/chiefdoms";
  return mdFetch(url, { method: "GET" });
}
export async function createChiefdom(payload: {
  name: string;
  districtId: number;
}) {
  return mdFetch("/api/master-data/chiefdoms", {
    method: "POST",
    body: JSON.stringify(payload),
  });
}
export async function updateChiefdom(
  id: number,
  payload: { name: string; districtId: number },
) {
  return mdFetch(`/api/master-data/chiefdoms/${id}`, {
    method: "PUT",
    body: JSON.stringify(payload),
  });
}
...
export async function createValueChain(payload: any) {
  return mdFetch("/api/master-data/value-chains", {
    method: "POST",
    body: JSON.stringify(payload),
  });
}
export async function updateValueChain(
  ...
  return mdFetch(`/api/master-data/value-chains/${id}`, {
    method: "PUT",
    body: JSON.stringify(payload),
  });
}
export async function toggleValueChain(id: number) {
  return mdFetch(`/api/master-data/value-chains/${id}/toggle`, {
    method: "PATCH",
  });
}
...
export async function createWarehouse(payload: any) {
  return mdFetch("/api/master-data/warehouses", {
    method: "POST",
    body: JSON.stringify(payload),
  });
}
export async function updateWarehouse(id: number, payload: any) {
  return mdFetch(`/api/master-data/warehouses/${id}`, {
    method: "PUT",
    body: JSON.stringify(payload),
  });
}
export async function toggleWarehouse(id: number) {
  return mdFetch(`/api/master-data/warehouses/${id}/toggle`, {
    method: "PATCH",
  });
}
export async function importWarehouses(rows: any[]) {
  return mdFetch("/api/master-data/warehouses/import", {
    method: "POST",
...
export async function createDistributionSite(payload: any) {
  return mdFetch("/api/master-data/distribution-sites", {
    method: "POST",
    body: JSON.stringify(payload),
  });
}
export async function updateDistributionSite(id: number, payload: any) {
  return mdFetch(`/api/master-data/distribution-sites/${id}`, {
    method: "PUT",
    body: JSON.stringify(payload),
  });
}
export async function toggleDistributionSite(id: number) {
  return mdFetch(`/api/master-data/distribution-sites/${id}/toggle`, {
    method: "PATCH",
  });
}
export async function importDistributionSites(rows: any[]) {
  return mdFetch("/api/master-data/distribution-sites/import", {
    method: "POST",
    body: JSON.stringify({ rows }),
  });
}
...
export async function createInputItem(payload: any) {
  return mdFetch("/api/master-data/input-items", {
    method: "POST",
    body: JSON.stringify(payload),
  });
}
export async function updateInputItem(id: number, payload: any) {
  return mdFetch(`/api/master-data/input-items/${id}`, {
    method: "PUT",
    body: JSON.stringify(payload),
  });
}
export async function toggleInputItem(id: number) {
  return mdFetch(`/api/master-data/input-items/${id}/toggle`, {
    method: "PATCH",
  });
}
...
export async function saveSystemSettings(updates: Record<string, string>) {
  return mdFetch("/api/master-data/system-settings", {
    method: "PUT",
...
export async function getFaceUploadUrl(
  farmerId: number,
  purpose: "reference" | "delivery",
  ...
  if (!res.ok) {
    const e = await res.json().catch(() => ({}));
    throw new Error((e as any).error ?? "Failed to get upload URL");
  }
  return res.json();
}
...
export async function saveFaceReference(
  farmerId: number,
  key: string,
): Promise<void> {
  const token = await getApiToken();
  ...
  if (!res.ok) {
    const e = await res.json().catch(() => ({}));
    throw new Error((e as any).error ?? "Failed to save reference");
  }
}
...
export async function getFaceViewUrl(key: string): Promise<string> {
  ...
  const res = await fetch(
    apiUrl(`/api/face/view-url?key=${encodeURIComponent(key)}`),
    {
      headers: { Authorization: `Bearer ${token}` },
    },
  );
  if (!res.ok) {
    const e = await res.json().catch(() => ({}));
    throw new Error((e as any).error ?? "Failed to get view URL");
  }
...
export async function getPhotoUrl(key: string): Promise<string | null> {
  try {
    return await getFaceViewUrl(key);
  } catch {
    return null;
  }
}
...
export async function analysePhotoLabels(s3Key: string): Promise<{
  ...
  if (!res.ok) {
    const e = await res.json().catch(() => ({}));
    throw new Error((e as any).error ?? "Label analysis failed");
  }
  return res.json();
}

export async function analyseFarmerFace(
  s3Key: string,
  farmerId: number,
): Promise<{
  ...
  if (!res.ok) {
    const e = await res.json().catch(() => ({}));
    throw new Error((e as any).error ?? "Face analysis failed");
  }
  return res.json();
}
...
export async function getAlertCounts(): Promise<{
  ...
  return {
    pendingFarmers: farmers.count ?? 0,
    pendingPod: pod.count ?? 0,
...
export async function listIncidents(page = 1, limit = 50, status?: string) {
  ...
  return {
    data: rows.map((r: any) => ({
      ...cc(r),
      officerName:
        officerMap[r.field_officer_id]?.full_name ?? r.reported_by ?? "Unknown",
    })),
    total: count ?? 0,
  };
}
...
export async function approvePod(
  id: number,
  forceApprove = false,
): Promise<{ duplicate?: boolean; existingPod?: any }> {
  const token = await podToken();
  ...
  if (resp.status === 409) {
    ...
    if ((data as any).duplicate)
      return { duplicate: true, existingPod: (data as any).existingPod };
  }
  if (!resp.ok) {
    const err = await resp.json().catch(() => ({ error: resp.statusText }));
...
export async function flagPodException(
  id: number,
  notes?: string,
): Promise<void> {
  const token = await podToken();
...
export async function overrideFacePod(
  id: number,
  reason: string,
): Promise<void> {
  const {
  ...
  if (!resp.ok) {
    const err = await resp.json().catch(() => ({ error: resp.statusText }));
    throw new Error(
      (err as any).error ?? "Failed to override face verification",
    );
  }
}
...
export async function bulkCheckDuplicates(
  phones: string[],
): Promise<Array<{ phone: string; farmerCode: string; name: string }>> {
  const {
    data: { session },
  } = await supabase.auth.getSession();
  const token = session?.access_token;

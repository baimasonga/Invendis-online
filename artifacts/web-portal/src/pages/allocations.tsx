...
function EditAllocationModal({
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    try {
      await updateMut.mutateAsync({ notes: notes || undefined });
      ...
      <DialogContent className="sm:max-w-sm" onAnimationStart={handleOpen}>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 py-1">
          ...
          <p className="text-xs text-muted-foreground">
            Delivery status changes only after a verified proof of delivery.
          </p>
...
export default function Allocations() {
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: number) => removeAllocation(id),
  });
    ...
    <div className="space-y-5">
      ...
      <Card>
        <CardHeader className="pb-0 pt-4 px-4">
          {!isLoading && (
            <p className="text-xs text-muted-foreground">
              {total.toLocaleString()} allocations
            </p>
          )}
        </CardHeader>
          ...
          <Table>
            <TableHeader>
              <TableRow className="hover:bg-transparent border-t">
                ...
                {can.manageAllocations && (
                  <TableHead className="pr-4 text-right w-[100px]" />
                )}
              </TableRow>
            </TableHeader>
            <TableBody>
              {isLoading ? (
                ...
                <TableRow>
                  <TableCell
                    colSpan={can.manageAllocations ? 6 : 5}
                    ...
                    <div className="flex flex-col items-center gap-2 text-muted-foreground">
                      ...
                      {can.manageAllocations && (
                        <Button
                          ...
                          onClick={() => setOpen(true)}
                        >
                          <Plus className="h-3.5 w-3.5 mr-1.5" /> Add first
                          allocation
                        </Button>
                      )}
                    </div>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              {deleteTarget?.farmerName ?? "this farmer"}'s allocation from{" "}
              <span className="font-medium">{deleteTarget?.campaignName}</span>.

...
export default function Dashboard() {
  ...
  const farmerChartData = (data?.charts?.farmerStatusChart ?? []).map((entry: any, i: number) => ({
    ...entry, fill: COLORS[i % COLORS.length],
  }));
  ...
  return (
    <div className="space-y-6">
      ...
      {isLoading ? (
        ...
        <>
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            <StatCard title="Total Farmers"     value={Number(summary.totalFarmers).toLocaleString()}     sub={`${Number(summary.pendingFarmers)} pending approval`} icon={Users} />
            ...
            {isLoading ? (
              ...
              <ResponsiveContainer width="100%" height={180}>
                <BarChart data={farmerChartData} layout="vertical" barSize={18}>
                  ...
                  <Tooltip cursor={{ fill: "rgba(0,0,0,0.04)" }} contentStyle={{ borderRadius: 8, fontSize: 12, border: "1px solid #e5e7eb" }} />
                  <Bar dataKey="value" radius={[0, 4, 4, 0]}>
                    {farmerChartData.map((entry: any) => <Cell key={entry.name} fill={entry.fill} />)}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
            ) : (
        ...
        <Card className="lg:col-span-2">
          ...
          <CardContent>
            {isLoading ? (
              ...
              <ResponsiveContainer width="100%" height={180}>
                <LineChart data={podTrendData} margin={{ top: 4, right: 12, left: -20, bottom: 0 }}>
                  <XAxis dataKey="date" tick={{ fontSize: 11 }} />
                  <YAxis tick={{ fontSize: 11 }} allowDecimals={false} />
                  <Tooltip contentStyle={{ borderRadius: 8, fontSize: 12, border: "1px solid #e5e7eb" }} />
                  <Legend wrapperStyle={{ fontSize: 11 }} />
                  <Line type="monotone" dataKey="verified" stroke="#15803d" strokeWidth={2} dot={{ r: 3 }} name="Verified" />
      ...
      {/* Charts row 2: Warehouse Stock + Campaign Status + Recent Activity */}
      <div className="grid gap-5 lg:grid-cols-3">
        <Card className="lg:col-span-1">
          <CardHeader className="pb-2">

...
-- ── USERS (legacy operational identity used by server workflows) ──────────
...
-- One PoD can contain one or more delivered input items. These bigint keys
-- match production and support the atomic submit/approval routines.
CREATE TABLE IF NOT EXISTS pod_items (
  id                 bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  pod_id             bigint NOT NULL REFERENCES pod(id) ON DELETE CASCADE,
  input_item_id      bigint NOT NULL REFERENCES input_items(id),
  quantity_delivered numeric NOT NULL,
  created_at         timestamptz NOT NULL DEFAULT now()
);
CREATE INDEX IF NOT EXISTS pod_items_pod_id_idx ON pod_items (pod_id);
CREATE INDEX IF NOT EXISTS pod_items_input_item_id_idx ON pod_items (input_item_id);
